/**
 * useCategorySelector - Database-Backed Version
 * 
 * ✅ MIGRATED: Now uses database queries instead of hardcoded CATEGORY_DEFINITIONS
 * 
 * This hook manages category selection state and fetches data from the database.
 * All data is cached using React Query for optimal performance.
 */
...
} from "../../../hooks/useHierarchyQueries";
...
interface UseCategorySelectorReturn {
  ...
  restrictedDivision: string | null;
...
export const useCategorySelector = (): UseCategorySelectorReturn => {
  ...
  useEffect(() => {
    if (userRole === 'CREATOR') {
      if (userDivision && state.selectedDepartment !== userDivision) {
        handleDepartmentChange(userDivision);
      }
      if (userSubDivision && state.selectedSubDepartment !== userSubDivision) {
        handleSubDepartmentChange(userSubDivision);
      }
    }
  }, [userRole, userDivision, userSubDivision, state.selectedDepartment, state.selectedSubDepartment, handleDepartmentChange, handleSubDepartmentChange]);
